  ...
  return (
    <div className="space-y-24">
      ...
      <motion.section
        ...
        className="relative bg-linear-to-br from-red-600 to-yellow-900 p-16 rounded-3xl shadow-xl overflow-hidden border border-red-700"
      >
        <div className="relative z-10 text-center space-y-4">
          <h1 className="text-5xl font-extrabold text-white drop-shadow">
           Os melhores gatinhos da semana
          </h1>

          <p className="text-blue-200 text-lg max-w-xl mx-auto">
            Gatos fofos, gatos peludinhos, gatos macios.
          </p>
      ...
      <section className="fade-in space-y-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 ls:grid-cols-3 gap-6">
          {[
            { nome: "Duplinha", img: "/images/produto1.jpg" },
            { nome: "Frajola", img: "/images/produto2.jpg" },
            { nome: "Laranjo", img: "/images/produto3.jpg" },
          ].map((cat, i) => (
            <motion.div
              key={i}
              whileHover={{ scale: 1.04 }}
              className="bg-yellow-600 p-4 rounded-xl border border-orange-700 shadow hover:border-red-500 transition cursor-pointer"
            >
              <img
                src={cat.img}
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-6 fade-in">
        {[
          ...
          <motion.div
            key={i}
            whileHover={{ scale: 1.04 }}
            className="bg-yellow-800 p-6 rounded-2xl border border-yellow-950 shadow text-center"
          >
            <h3 className="text-xl font-bold text-slate-100 mb-2">
              {info.title}
            </h3>
            <p className="text-slate-300">{info.desc}</p>
          </motion.div>
        ))}
      </section>
    </div>
